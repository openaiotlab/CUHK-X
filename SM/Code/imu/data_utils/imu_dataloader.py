...
class IMU_DataLoader(Dataset):
    def __init__(self,
        ...
        self.csv_dir = os.path.dirname(os.path.abspath(datapth))
        self.data_root = os.environ.get('CUHKX_IMU_ROOT')
    ...
    def resolve_data_path(self, path_value):
        ...
        candidates = []
        if self.data_root:
            candidates.append(os.path.join(self.data_root, path_str))
        candidates.append(os.path.join(self.csv_dir, path_str))
    ...
    def get_data_for_instance(self,instance,modality):
        up_data=pd.read_csv(self.resolve_data_path(instance['up_path'].iloc[0]))
        down_data=pd.read_csv(self.resolve_data_path(instance['down_path'].iloc[0]))
