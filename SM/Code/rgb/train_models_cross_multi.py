...
# ---------- Arg parsing helpers ----------
def parse_user_ids(s: Optional[str]) -> Optional[List[int]]:
    """
    Parse user ID(s) from string.
    Accepts: "5" (single), "1,2,3,5,10" (multiple comma-separated)
    ...
    if not s:
        return None
    try:
        user_ids = [int(x.strip()) for x in s.split(',') if x.strip()]
        return user_ids if user_ids else None
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            f"Invalid user ID format: {s}. Expected single ID or comma-separated list."
        ) from exc
...
def load_data(
    ...
    Args:
        test_user_ids: List of user IDs for test set in cross mode (e.g., [1,2,3,5,10])
    Returns:
    ...
    if split_mode == "cross":
        if test_user_ids is None:
            raise ValueError("test_user_ids must be provided for 'cross' mode.")
        test_user_set = set(test_user_ids)
        train_data = [item for item in mapped_data if item[2] not in test_user_set]
        test_data = [item for item in mapped_data if item[2] in test_user_set]
        logging.info("Cross mode: Using users %s for testing, remaining users for training.", test_user_ids)
...
def main():
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default=os.environ.get("CUHKX_MULTIMODAL_DATA_ROOT", "/path/to/CUHK-X/SM/Data"),
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="under cross mode the testing user ID(s). Can be single (e.g., '5') or multiple comma-separated (e.g., '1,2,3,5,10')",
    )
    args = parser.parse_args()
    labels_range = parse_labels_arg(args.labels)
    test_user_ids = parse_user_ids(args.cross_user_id)
    
    # Validate cross_user_id for cross mode
    if args.split_mode == "cross" and test_user_ids is None:
        parser.error("--cross_user_id is required when --split_mode is 'cross'")
    ...
    for epoch in range(args.epochs):
        ...
        num_batches = 0
        train_loop = tqdm(
            train_dataloader, desc=f"Epoch {epoch + 1}/{args.epochs}", leave=True
        )
        ...
        print(f"Epoch {epoch + 1}/{args.epochs} - Training Loss: {avg_train_loss:.4f}", flush=True)
