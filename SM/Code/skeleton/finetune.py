...
CUDA_VISIBLE_DEVICES=3 python finetune.py --train_dir cross_trial_train --test_dir cross_trial_test --config configs/dstformer.yaml 
CUDA_VISIBLE_DEVICES=3 python finetune.py --train_dir cross_trial_train.txt --test_dir cross_trial_test.txt --config configs/dstformer.yaml 
...
def train_with_config(args, opts):
    ...
    checkpoint = torch.load('checkpoint/motionbert_best_epoch.bin', map_location=lambda storage, loc: storage)
    # Remove the final fully connected (fc/head) layer weights before loading
    state_dict = checkpoint['model']
    fc_keys = [k for k in state_dict.keys() if 'module.head' in k]
    for k in fc_keys:
    ...
    train_dataset = cuhkxDataset(data_split='split/'+opts.train_dir+'.txt', scale_range=args.scale_range_train)
    val_dataset = cuhkxDataset(data_split='split/'+opts.test_dir+'.txt', scale_range=args.scale_range_test)
    

    train_loader = DataLoader(train_dataset, **trainloader_params)
    test_loader = DataLoader(val_dataset, **testloader_params)

    opts.checkpoint = 'checkpoint/cuhkx_cross_trial_pretrained/'+ args.backbone + '_' + opts.train_dir
    ...
    if not opts.evaluate:
        optimizer = optim.AdamW(
            filter(lambda p: p.requires_grad, model.module.head.parameters()),
            lr=args.lr_head,
...
if __name__ == "__main__":
    ...
    if not opts.evaluate:
        wandb.init(project="cuhkx_cross_trial", name=args.backbone+'_'+opts.train_dir+'_pretrained_fc_finetune')
